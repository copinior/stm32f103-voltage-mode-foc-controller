...
实现包含电角度对齐和 SVPWM 调制，但不包含相电流采样与真实电流闭环。若使用上游语音识别，则前端由单独的 ESP32 / ESP8266 项目负责。
...
5. `motor_control_service` 更新电压型 FOC 主循环当前使用的控制模式
6. `TelemetryTask` 在收到请求时打印电机状态

## 语音命令映射

当前语音桥接支持命令 ID `0..15`，用于与上游语音识别工程保持一致。其中命令 `0` 和 `1` 当前仅作为保留命令位，在 STM32 侧不参与应用层动作分发。
...
- 当前实现不使用相电流采样
- 现有“力矩”相关路径本质上是电压指令路径，而不是真实电流闭环调节
- 本项目的主要展示点在于受限传感条件下的多环控制架构设计与工程实现
...
当关闭语音桥接时，工程仍可通过本地 UART 控制台进行电机控制和状态打印。
